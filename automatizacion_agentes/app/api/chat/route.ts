...
6. Email de contactoimport { NextRequest, NextResponse } from "next/server";
import { Resend } from "resend";
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ─── FIX: Gemini exige que 'contents' empiece siempre por rol 'user' ───────
    // El saludo inicial del bot llega como primer mensaje con rol 'assistant',
    // lo que hace que Gemini devuelva INVALID_ARGUMENT. Lo eliminamos.
    const contents = messages
      .map((m: { role: string; content: string }) => ({
        role: m.role === "assistant" ? "model" : "user",
        parts: [{ text: m.content }],
      }))
      .filter((_, i, arr) => !(i === 0 && arr[0].role === "model")); // quitar saludo inicial si es 'model'

    if (contents.length === 0) {
      return NextResponse.json({ error: "No hay mensajes válidos para procesar" }, { status: 400 });
    }
    // ─────────────────────────────────────────────────────────────────────────────
